...
.header-icon {
  ...
  font-size: 20px; box-shadow: 0 0 20px rgba(79,158,255,.3);
}
...
.btn-success  { background: linear-gradient(135deg, var(--accent3), #00b87c); color: #0a0c10; box-shadow: 0 2px 12px rgba(0,229,176,.2); }
...
.btn-danger   { background: rgba(255,77,106,.12); color: var(--danger); border: 1px solid rgba(255,77,106,.2); }
...
.filter-title { font-size: 11px; color: var(--text3); text-transform: uppercase; letter-spacing: 1px; margin-bottom: 14px; font-family: var(--font-mono); }
...
.filter-group select:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(79,158,255,.12); }
...
.th-group {
  background: var(--bg); text-align: center; font-size: 10px; font-weight: 800;
...
td { padding: 10px 12px; color: var(--text2); white-space: nowrap; font-family: var(--font-mono); font-size: 12px; }
td.td-codigo  { color: var(--accent); font-weight: 600; }
td.td-desc    { color: var(--text); font-family: var(--font-main); font-size: 12.5px; max-width: 200px; overflow: hidden; text-overflow: ellipsis; }
td.td-fecha   { color: var(--text2); }
td.td-tipo-op { color: var(--accent3); font-size: 11px; font-weight: 600; }
...
.badge { display: inline-block; padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 600; font-family: var(--font-main); }
...
.pagination {
  display: flex; align-items: center; justify-content: center;
  gap: 6px; padding: 16px; border-top: 1px solid var(--border); flex-wrap: wrap;
}
.page-btn {
...
.nav-item { display: flex; flex-direction: column; align-items: center; gap: 3px; text-decoration: none; opacity: .5; transition: opacity .15s; padding: 4px 12px; }
.nav-item.active { opacity: 1; }
.nav-icon  { font-size: 20px; }
.nav-label { font-size: 9px; font-weight: 700; color: var(--text2); letter-spacing: .5px; text-transform: uppercase; }
.nav-item.active .nav-label { color: var(--accent); }

.mobile-card { display: none; background: var(--bg2); border: 1px solid var(--border); border-radius: var(--radius); margin-bottom: 8px; overflow: hidden; }
...
@media (max-width: 768px) {
  ...
  .header-actions { width: 100%; display: none; }
...
.btn-danger-outline:hover { background: rgba(255,77,106,.18); border-color: var(--danger); transform: translateY(-1px); }
...
<div class="modal-overlay" id="modalOverlay">
  <div class="modal">
    ...
    <div class="modal-actions">
      ...
      <a href="#" id="modalConfirm" class="btn btn-danger-outline" style="justify-content:center">🗑 Sí, eliminar</a>
    </div>
  </div>
</div>
...
<nav class="bottom-nav">
  <div class="bottom-nav-inner">
    <a href="index.php" class="nav-item active"><span class="nav-icon">📦</span><span class="nav-label">Kardex</span></a>
    <a href="importar.php" class="nav-item"><span class="nav-icon">⬆</span><span class="nav-label">Importar</span></a>
    <a href="reporte.php<?= $search_codigo || $search_fecha_ini || $search_fecha_fin ? '?codigo='.urlencode($search_codigo).'&fecha_ini='.urlencode($search_fecha_ini).'&fecha_fin='.urlencode($search_fecha_fin) : '' ?>" class="nav-item"><span class="nav-icon">📄</span><span class="nav-label">Reporte</span></a>
